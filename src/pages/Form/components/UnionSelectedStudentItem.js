      ...
      <div className="table-responsive" hidden={collapse}>
        <Table stickyHeader aria-label="sticky table" className="table table-hover text-nowrap mb-0">
          <thead>
            <tr>
              <th>
                <StyledCheckbox checked={selectedCheckAll.some(v => v === item.team)} onChange={handleCheckAll} />
              </th>
            ...
            {[...item.students]
              ...
              .map(student => (
                <tr key={`student-row-${student.id}`}>
                  <td>
                    <StyledCheckbox onChange={handleClickRow} value={student.id} checked={selectedIds.some(v => v === student.id)} />
                  </td>
                  <td>
