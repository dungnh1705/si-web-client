        ...
        <div className='card-header--title'>
          ...
          {item.team !== 0 && (
            <h4 className='font-size-lg mb-0 py-2 font-weight-bold'>
              Đội: {item.team} - {item?.students?.length} | {item?.students?.filter(i => i.status !== StudentStatus.Active)?.length} | {item?.students?.filter(i => i.studentClass[0].stayInClass)?.length}
            </h4>
          )}
        </div>
        ...
        <table className='table table-hover text-nowrap mb-0'>
          ...
          {item.students.length > 0 &&
            [...item.students]
              ...
              .map((stu, index) => <StudentTeamItem key={`stu-class-${stu.id}-${index}`} student={stu} team={item.team}
                                                    viewAbsentMode={false} index={index + 1} />)}
          </tbody>
        </table>
      </div>
    </Card>
